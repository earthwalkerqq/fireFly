...
#define DELAUNAY_NONE ((uint32_t)-1)
...
/**
 * @brief Сохраняет индексы половины ребер треугольника t в edges
 * @param [in] uint32_t t - треугольник
 * @param [out] uint32_t edges[3] - половины ребер треугольника
 */
static inline void
...
/**
 * @return возвращает идентификатор треугольника с половиной ребра e
 */
...
/**
 * @return функции tri_next_edge() и tri_prev_edge() возвращают значение следующего или предыдущего ребра
 * в треугольнике
 */
static inline uint32_t
                 ...
                 void *arg)
{
    ...
    for (uint32_t incoming = 0; incoming < ntriverts; ++ incoming) {
        ...
        if (halfedge[endpoint] == DELAUNAY_NONE) continue;
        ...
        } while (current != DELAUNAY_NONE && current != incoming);
